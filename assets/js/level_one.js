document.addEventListener("DOMContentLoaded", function () {
    ...
    var best_score = localStorage.getItem("bestScore_levelOne");
...
// startTimer(60);  // remove forward slashes in front of startTimer to repeat if required
            if(localStorage.getItem("bestScore_levelOne") < score_number.innerHTML){
                localStorage.setItem("bestScore_levelOne", score_number.innerHTML);
            }
        }
    ...
    function PressingKey(eve) {
        eve.preventDefault();

        function keyAction(secs,char) {
            var secs = timeInSecs;
            if (secs == 0){
                console.log("TIME OUT ! Votre score est :" + score_number.innerHTML);
            }else {
                console.log(char);
                var select = document.querySelectorAll(".char." + char).length;
                console.log(select);
                if(select > 0){
                    score_number.innerHTML++;
                    var div = document.querySelector(".char." + char);
                    div.remove();
                }else {
                    score_number.innerHTML--;
                }
